# MASTER PROMPT — OneNote-Import Jahr 2 (Unterstufe, LG Vaduz)
...
überführt. Dieses Fach-Jahr: **Jahr 2, Unterstufe**. Der Wert dieser App
...
- **Curriculum-Archivar** — kennt `data/curriculum/*.json` lückenlos, weiss exakt was laut LG-Lehrplan in Jahr 2 drankommt.
...
1. Für jedes Fach unten: OneNote-Abschnitt für Jahr 2 als **PDF** exportieren (Datei → Exportieren → PDF, **ganzer Abschnitt**, nicht einzelne Seiten).
2. Ablegen unter: `data/onenote-inbox/Jahr-2/<Fach>.pdf` — `<Fach>` exakt wie in der Fächerliste unten.
...
## Fächerliste Jahr 2 (Unterstufe — alle Grundlagenfächer)
...
(Falls laut Lehrplan ein Fach in Jahr 2 real noch nicht unterrichtet wird —
...
- Bestätige die Fächerliste für Jahr 2 gegen `grundlagenfaecher` in `curriculum.json`.
...
- Scanne `data/subjects/<Fach>/2/*.md` für bereits bestehende Themen (aktuell nur `Mathematik/2/lineare-funktionen.md` befüllt) — nicht duplizieren, nur ergänzen/verbessern.
...
Für jedes Fach mit Dateien in `data/onenote-inbox/Jahr-2/<Fach>.*`:
...
**Thema aus echten Notizen belegt** → `data/subjects/<Fach>/2/<thema-slug>.md`, **ohne** `-beta`-Suffix. Erste Zeile nach dem Titel:
```
> ✅ Aus deinen echten OneNote-Notizen (Jahr 2) übernommen und aufbereitet.
```

**Thema steht laut Checkliste an, aber in KEINER Quelle vorhanden** (auch: ganzes Fach ohne Inbox-Material) → recherchiere strikt anhand des `faecher.json`-Eintrags + seriöser Quellen, **nur** im Rahmen dessen, was der Lehrplan für dieses Fach/Jahr vorschreibt, nichts darüber hinaus erfinden. Datei: `data/subjects/<Fach>/2/<thema-slug>-beta.md`. Erste Zeile:
```
...
### Schritt 4 — Bericht
Schreibe `data/onenote-inbox/Jahr-2/SCRAPE-REPORT.md`:
