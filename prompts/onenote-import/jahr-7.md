# MASTER PROMPT — OneNote-Import Jahr 7 (Oberstufe, Maturajahr, LG Vaduz)
...
```
↑ Angenommen aus Klasse „5Wa" (`kurz: W`). War/ist dein Profil in Jahr 7 ein
...
überführt. Dieses Fach-Jahr: **Jahr 7, Oberstufe — das Maturajahr**. Höchste
Sorgfaltsstufe: dieses Material ist die letzte Instanz vor der Matura, hier
zählt jedes fehlende Detail doppelt. Der Wert dieser App steht und fällt mit
echten Notizen aus echten LG-Stunden — kein generisches Internet-Allerlei,
wo echtes Material existiert.

### Dein Team für diese Session
- **Curriculum-Archivar** — kennt `data/curriculum/*.json` lückenlos, weiss exakt was laut LG-Lehrplan in Jahr 7 (Grundlagen + Profil `PROFIL`) drankommt, inklusive Maturastoff-Schwerpunkte.
...
1. Für jedes Fach unten: OneNote-Abschnitt für Jahr 7 als **PDF** exportieren (Datei → Exportieren → PDF, **ganzer Abschnitt**, nicht einzelne Seiten).
2. Ablegen unter: `data/onenote-inbox/Jahr-7/<Fach>.pdf` — `<Fach>` exakt wie in der Fächerliste unten.
...
## Fächerliste Jahr 7 (Oberstufe — Grundlagen + Profil)
...
**+ Profilfächer von `PROFIL`** (bei `wirtschaft-recht`, Stufe 7:
**Integrationsfach Betriebswirtschaftslehre** — 2 Lektionen — und
**Volkswirtschaftslehre** — 3 Lektionen).
...
Stufe 5) laufen in Stufe 7 **nicht** — dafür keine Dateien/Ordner
anlegen. **Zusätzliche Unsicherheit, vor dem Lauf prüfen:** Das
GRUNDLAGENFACH „Wirtschaft und Recht" (nicht das Profilfach) hat laut
`faecher.json` in Stufe 7 selbst ebenfalls 2 Lektionen (das ist die
Dotation für Schüler OHNE dieses Profil) — ungeklärt ist, ob
Profilschüler wie Jayden dieses Grundlagenfach in Stufe 7 zusätzlich zum
Profilfach belegen. Falls unklar: bei Jayden nachfragen statt zu raten,
bevor ein eigener „Wirtschaft und Recht"-Ordner für Jahr 7 angelegt wird.
Falls `PROFIL` oben geändert wurde: die passenden `profilfaecher` direkt
aus `curriculum.json` lesen, nicht diese Liste hier verwenden.

(Falls laut Lehrplan ein Fach in Jahr 7 real nicht unterrichtet wird,
...
- Berechne die exakte Fächerliste für Jahr 7: `grundlagenfaecher` + `profile.find(p => p.id === PROFIL).profilfaecher` (Duplikate raus) — analog zur Logik in `src/lib/curriculum.ts::subjectsFor`. **Achtung:** `profilfaecher` liefert nur Namen, keine Stufen-Zuordnung — `subjectsFor()` gibt sie für jedes Oberstufenjahr zurück, obwohl sie es real nicht sind. Prüfe darum für JEDES Profilfach-Kandidat zusätzlich in `faecher.json` unter „1. Lektionendotation", ob die Stufe-7-Spalte > 0 ist, bevor du eine Datei dafür anlegst.
- Lies pro betroffenem Fach den zugehörigen Eintrag in `faecher.json` (Beschreibung, Richtziele, Stoffprogramm) — bei Profilfächern und Maturaschwerpunkten besonders genau. Das ist deine Checkliste, welche Themen in diesem Jahr vorkommen müssen.
- Scanne `data/subjects/<Fach>/7/*.md` für bereits bestehende Themen (aktuell noch keine Datei für Jahr 7 vorhanden — reiner Neuaufbau) — trotzdem zuerst prüfen, nicht blind überschreiben, falls sich das inzwischen geändert hat.
...
Für jedes Fach mit Dateien in `data/onenote-inbox/Jahr-7/<Fach>.*`:
...
**Thema aus echten Notizen belegt** → `data/subjects/<Fach>/7/<thema-slug>.md`, **ohne** `-beta`-Suffix. Erste Zeile nach dem Titel:
```
> ✅ Aus deinen echten OneNote-Notizen (Jahr 7) übernommen und aufbereitet.
```

**Thema steht laut Checkliste an, aber in KEINER Quelle vorhanden** (auch: ganzes Fach ohne Inbox-Material) → recherchiere strikt anhand des `faecher.json`-Eintrags + seriöser Quellen, **nur** im Rahmen dessen, was der Lehrplan für dieses Fach/Jahr vorschreibt, nichts darüber hinaus erfinden. Datei: `data/subjects/<Fach>/7/<thema-slug>-beta.md`. Erste Zeile:
```
> 🧪 **Beta** — nicht in deinen OneNote-Notizen/Zusammenfassungen von Jahr 7 gefunden. Aus dem offiziellen LG-Lehrplan + ergänzender Recherche erstellt. Ersetzen/ergänzen, sobald echte Notizen vorliegen — gerade im Maturajahr besonders wichtig.
```
...
### Schritt 4 — Bericht
Schreibe `data/onenote-inbox/Jahr-7/SCRAPE-REPORT.md`:
...
- Klare To-Do-Liste: welche Fächer/Themen du noch mit echten Notizen nachliefern solltest — im Maturajahr priorisiert nach Prüfungsrelevanz, falls aus dem Lehrplan ersichtlich.
