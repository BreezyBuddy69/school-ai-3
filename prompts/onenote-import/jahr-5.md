# MASTER PROMPT — OneNote-Import Jahr 5 (Oberstufe, LG Vaduz)
...
```
↑ Angenommen aus Klasse „5Wa" (`kurz: W`) — das ist vermutlich dein
**aktuelles** Schuljahr. War dein Profil in Jahr 5 ein anderes? Ändere die
Zeile auf einen der `profile[].id`-Werte aus
...
überführt. Dieses Fach-Jahr: **Jahr 5, Oberstufe**. Der Wert dieser App
...
- **Curriculum-Archivar** — kennt `data/curriculum/*.json` lückenlos, weiss exakt was laut LG-Lehrplan in Jahr 5 (Grundlagen + Profil `PROFIL`) drankommt.
...
1. Für jedes Fach unten: OneNote-Abschnitt für Jahr 5 als **PDF** exportieren (Datei → Exportieren → PDF, **ganzer Abschnitt**, nicht einzelne Seiten).
2. Ablegen unter: `data/onenote-inbox/Jahr-5/<Fach>.pdf` — `<Fach>` exakt wie in der Fächerliste unten.
...
## Fächerliste Jahr 5 (Oberstufe — Grundlagen + Profil)
...
**+ Profilfächer von `PROFIL`** (bei `wirtschaft-recht`, Stufe 5:
**Integrationsfach Betriebswirtschaftslehre** — 4 Lektionen — und
**Rechtskunde** — 2 Lektionen, nur in Stufe 5).
...
„Finanzbuchhaltung" (nur Stufe 4) und „Volkswirtschaftslehre" (erst Stufe
6–7) laufen in Stufe 5 **nicht** — dafür keine Dateien/Ordner anlegen. Der
erste Jahr-5-Lauf (2026-07-21) wurde noch mit den falschen Namen
durchgeführt und danach manuell nach `data/subjects/Integrationsfach
Betriebswirtschaftslehre/5/` bzw. `data/subjects/Rechtskunde/5/`
verschoben — bei einem erneuten Lauf diese (korrekten) Ordner verwenden,
nicht die alten Namen „Wirtschaft und Recht"/„Finanz- und
Rechnungswesen"/„Volkswirtschaftslehre" für Jahr 5 wiederbeleben. Falls
`PROFIL` oben geändert wurde: die passenden `profilfaecher` direkt aus
`curriculum.json` lesen, nicht diese Liste hier verwenden.

(Falls laut Lehrplan ein Fach in Jahr 5 real nicht unterrichtet wird,
...
- Berechne die exakte Fächerliste für Jahr 5: `grundlagenfaecher` + `profile.find(p => p.id === PROFIL).profilfaecher` (Duplikate raus) — analog zur Logik in `src/lib/curriculum.ts::subjectsFor`. **Achtung:** `profilfaecher` liefert nur Namen, keine Stufen-Zuordnung — `subjectsFor()` gibt sie für jedes Oberstufenjahr zurück, obwohl sie es real nicht sind. Prüfe darum für JEDES Profilfach-Kandidat zusätzlich in `faecher.json` unter „1. Lektionendotation", ob die Stufe-5-Spalte > 0 ist, bevor du eine Datei dafür anlegst.
...
- Scanne `data/subjects/<Fach>/5/*.md` für bereits bestehende Themen (aktuell **`Philosophie/5/` bereits mit 8 Dateien befüllt** — die nicht duplizieren, nur ergänzen/verbessern; alle anderen Fächer in Jahr 5 sind noch leer).
- Nutze `fachSlugMap` aus `curriculum.json`, falls der Ordnername vom Fachnamen abweicht (z. B. Französisch → `Franzoesisch`, Philosophie und Ethik → `Philosophie`). Für neue Profilfächer ohne bisherigen Ordner (z. B. "Finanz- und Rechnungswesen") den Ordner beim Schreiben der ersten Datei einfach neu anlegen.

### Schritt 2 — Inbox verarbeiten (pro Fach mit Material)
Für jedes Fach mit Dateien in `data/onenote-inbox/Jahr-5/<Fach>.*`:
...
### Schritt 4 — Bericht
Schreibe `data/onenote-inbox/Jahr-5/SCRAPE-REPORT.md`:
